...
struct HomeAssistantService {
    ...
    func getEntities<Attributes: Decodable>(
        decoding attributesType: Attributes.Type,
        on client: any Client
    ) async throws -> [HomeAssistantEntity<Attributes>] {
        ...
        return try response.content.decode([HomeAssistantEntity<Attributes>].self)
    }

    func getEntity<Attributes: Decodable>(
        id: String,
        decoding attributesType: Attributes.Type,
        on client: any Client
    ) async throws -> HomeAssistantEntity<Attributes> {
        let response = try await client.get(URI(string: "\(try baseURL)/api/states/\(id)")) { request in
            request.headers.add(name: .authorization, value: "Bearer \(try token)")
        }

        guard response.status == .ok else {
            throw Abort(.badGateway, reason: "Home Assistant returned \(response.status.code) while fetching \(id)")
        }
...
struct HomeAssistantEntity<Attributes: Decodable>: Decodable {
    let entityId: String
    let state: String
